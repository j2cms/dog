...
//		conf.set("mapred.job.tracker", "lenovo0:9001");
	...
	public static void testDNS() throws Exception {
		...
		buildClusterer(input, basePath, 1.8, 4,2,false);
	}
...
//		String inputFile = basePath + "/agaricus-lepiota.data";
		...
		int p = 4;
		int maxIter = 1;
		buildClusterer(inputFile, basePath, r, p, maxIter,true);
	}
	...
	public static void testPeter() throws Exception{
		...
		int p = 3;
		int maxIter = 1;
		buildClusterer(inputFile, output, r, p, maxIter,false);
	}
...
//		 testMushroom();
		...
		if(args[5].equals("1")||args[5].equals("true")) 
			...
			num = false;
		buildClusterer(args[0], args[1], Double.valueOf(args[2]), Integer.valueOf(args[3]),Integer.valueOf(args[4]),num);
